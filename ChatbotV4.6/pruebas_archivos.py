...
prompt_modelo = st.text_area("Prompt que utiliza el modelo", config["prompt"])

#Contenedor para mostrar el historial de chat
chat_container = st.container(height=225)
...
if input:
    try:
        if not prompt_modelo:
            imprimirMensaje(f"El prompt inicial no puede quedar vacío")
        else:
            controlador.generar_respuesta(input,prompt_modelo, model_choice, TEST_MEMORY)
    except Exception as e:
